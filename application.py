...
def index():
    ...
    for i in port_data:
        ...
        i["gainloss"] = round(
            (((stock_data["price"] * i["shares"]) - i["total"]) / i["total"]) * 100, 2)
...
def buy():
    if request.method == "POST":
        ...
        #log transaction
        db.execute("INSERT INTO trans (user_id, symbol, shares, price, total, buy_sell, datetime) VALUES (:id, :sym, :sh, :pr, :tot, 'BUY', :dt)",
                   id=u_id, sym=stock, sh=quantity, pr=price, tot=total, dt=dt)

        #update portfolio
        if db.execute("SELECT symbol FROM port WHERE user_id = :id AND symbol = :stock", id=u_id, stock=stock):
            db.execute("UPDATE port SET shares = shares + :sh, last_price = :pr, total = total + :tot WHERE user_id = :id AND symbol = :stock",
                       id=u_id, stock=stock, sh=quantity, pr=price, tot=total)
        else:
            db.execute("INSERT INTO port (user_id, symbol, shares, last_price, total) VALUES (:id, :sym, :sh, :pr, :tot)",
                       id=u_id, sym=stock, sh=quantity, pr=price, tot=total)

        #exe new cash balance
        db.execute("UPDATE users SET cash = :newcash WHERE id = :id",
                   newcash=funds-total, id=u_id)
...
def register():
    ...
    if request.method == "POST":
        ...
        if db.execute("SELECT * FROM users WHERE username = :name", name=name):
            ...
            db.execute(
                "INSERT INTO users (username, hash) VALUES (:name, :hash)", name=name, hash=pw)

        #login user automatically
        rows = db.execute(
            "SELECT * FROM users WHERE username = :username", username=name)
...
def sell():
    ...
    if request.method == "POST":
        ...
        q_owned = db.execute(
            "SELECT shares FROM port WHERE user_id = :id AND symbol = :stock", id=u_id, stock=stock)
        ...
        #log transaction
        db.execute("INSERT INTO trans (user_id, symbol, shares, price, total, buy_sell, datetime) VALUES (:id, :sym, :sh, :pr, :tot, 'SELL', :dt)",
                   id=u_id, sym=stock, sh=quantity, pr=price, tot=total, dt=dt)
        ...
        # check quantity owned
        q_owned = db.execute(
            "SELECT shares FROM port WHERE user_id = :id AND symbol = :stock", stock=stock, id=u_id)
        q_owned = q_owned[0]["shares"]

        if q_owned == 0:
            db.execute(
                "DELETE FROM port WHERE user_id=:id AND symbol=:stock", stock=stock, id=u_id)

        #exe new cash balance
        db.execute(
            "UPDATE users SET cash = cash + :newcash WHERE id = :id", newcash=total, id=u_id)
...
def forgot():
    ...
    if request.method == "POST":
        ...
        #set new PW
        db.execute(
            "UPDATE users SET hash = :newhash WHERE username = :name", name=name, newhash=pw)

        #login user automatically
        rows = db.execute(
            "SELECT * FROM users WHERE username = :name", name=name)
